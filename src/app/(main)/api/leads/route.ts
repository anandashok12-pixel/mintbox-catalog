...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, company, email, items } = body
    if (!name || !company || !email || !items || items.length === 0) {
      return NextResponse.json({ error: 'Missing required fields: name, company, email, items' }, { status: 400 })
    }
